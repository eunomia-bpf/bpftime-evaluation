...
int sqlite3WalOpen(
  ...
  *ppWal = 0;
  ...
  /* huyp modified: init an io_uring queue */
  io_uring_queue_init(1000, &pRet->queue.queue, 0);
  pRet->queue.acquired_sqe = 0;
  pRet->queue.submitted_sqe = 0;
...
void sqlite3WalTryWait(uring_queue* uptr)
{
  if(uptr->submitted_sqe > 0)
  {
    struct io_uring_cqe* cqe = NULL;
    int ret = io_uring_peek_cqe(&uptr->queue, &cqe);
    if(ret == 0)
    {
      ...
      io_uring_cqe_seen(&uptr->queue, cqe);
      uptr->submitted_sqe -= 1;
    }
  }
}
...
int sqlite3WalClose(
  ...
  if( pWal ){
    ...
    */
    ...
    io_uring_queue_exit(&pWal->queue.queue);
...
int sqlite3WalFrames(
  ...
  ** nTruncate==0 then this frame set does not complete the transaction. */
  ...
  // huyp: First we wait for the before one
  // Remove this can get without io_uring_wait 99p latency
  if(pWal->queue.submitted_sqe!= 0)
  {
    sqlite3OsWait(pWal->pWalFd, &pWal->queue);
  }
  ...
  */
  ...
  if( iFrame==0 ){
    ...
    */
    if( pWal->syncHeader ){
      // huyp: change normal sync =>   io_uring sync

      // Second, remove origin sync, to io_uring sync and submit
      // rc = sqlite3OsSync(pWal->pWalFd, CKPT_SYNC_FLAGS(sync_flags));

      rc = sqlite3OsASync(pWal->pWalFd, CKPT_SYNC_FLAGS(sync_flags), &pWal->queue);
      if( rc ) return rc;
    }
  }
  ...
  */
  if( isCommit && WAL_SYNC_FLAGS(sync_flags)!=0 ){
    ...
    if( bSync ){
      ...
      // do same to the sync header just like above 
      rc = sqlite3OsASync(w.pFd, WAL_SYNC_FLAGS(sync_flags), &pWal->queue);
      rc = sqlite3OsSubmit(w.pFd, &pWal->queue);
    }
  }
