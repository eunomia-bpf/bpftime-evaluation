...
int sqlite3OsASync(sqlite3_file *id, int flags,uring_queue* uptr){
  DO_OS_MALLOC_TEST(id);
  return flags ? id->pMethods->xASync(id, flags, uptr) : SQLITE_OK;
}
int sqlite3OsTryWait(sqlite3_file *id, uring_queue* uptr){
  id->pMethods->xTryWait(id, uptr);
  return SQLITE_OK;
}
int sqlite3OsWait(sqlite3_file *id, uring_queue* uptr)
{
  return id->pMethods->xWait(id, uptr);
}

int sqlite3OsSubmit(sqlite3_file *id, uring_queue* uptr)
{
  return id->pMethods->xSubmit(id, uptr);
}
