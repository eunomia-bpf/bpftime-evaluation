...
static void setup_syscall_tracer()
{
	// Setup page mappings
	...
	*/
	...
	auto handle =
		dlmopen(LM_ID_NEWLM, "./hooker.so", RTLD_NOW | RTLD_LOCAL);
	if (!handle) {
		std::cerr << "Failed to open hooker lib: " << dlerror()
			  << std::endl;
		exit(1);
	}
	auto setup_hooker = (void (*)(syscall_hooker_func_t *))dlsym(
		handle, "setup_hooker");
	if (!setup_hooker) {
		std::cerr << "setup_hooker not found in hooker.so" << std::endl;
		exit(1);
	}
	setup_hooker(&call_hook);
}
