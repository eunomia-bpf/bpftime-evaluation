...
int tracepoint__syscalls__sys_enter_open(struct trace_event_raw_sys_enter *ctx)
{
	...
	if (trace_allowed(tgid, pid)) {
		...
		bpf_map_update_elem((void *)(uintptr_t)1, &pid, &args, 0);
	}
	return 0;
}
...
static __always_inline int trace_exit(struct trace_event_raw_sys_exit *ctx)
{
	...
	ap = bpf_map_lookup_elem((void *)(uintptr_t)1, &pid);
	if (!ap)
	...
	/* emit event */
	bpf_perf_event_output(ctx, (void *)(uintptr_t)2, BPF_F_CURRENT_CPU,
			      &event, sizeof(event));

cleanup:
	bpf_map_delete_elem((void *)(uintptr_t)1, &pid);
	return 0;
}
