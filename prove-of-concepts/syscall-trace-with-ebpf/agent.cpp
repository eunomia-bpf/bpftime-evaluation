...
static void setup_syscall_tracer(const char *ebpf_prog_path,
				 const char *hooker_path)
{
	// Setup page mappings
	...
	*/
	...
	auto handle = dlmopen(LM_ID_NEWLM, hooker_path, RTLD_NOW | RTLD_LOCAL);
	if (!handle) {
		std::cerr << "Failed to open hooker lib: " << dlerror()
			  << std::endl;
		exit(1);
	}
	auto ebpf_prog_path_in_hooker = (char *)dlsym(handle, "ebpf_prog_path");
	if (!ebpf_prog_path_in_hooker) {
		std::cerr << "ebpf_prog_path_in_hooker not found in hooker.so"
			  << std::endl;
		exit(1);
	}
	strcpy(ebpf_prog_path_in_hooker, ebpf_prog_path);
	auto setup_hooker =
		(int (*)(syscall_hooker_func_t *))dlsym(handle, "setup_hooker");
	if (!setup_hooker) {
		std::cerr << "setup_hooker not found in hooker.so" << std::endl;
		exit(1);
	}
	if (int err = setup_hooker(&call_hook); err < 0) {
		std::cerr << "Failed to setup hooker: " << err << std::endl;
		exit(1);
	}
}
...
extern "C" void agent_main(const gchar *data, gboolean *stay_resident)
{
	...
	*stay_resident = TRUE;
	...
	if (where == std::string::npos) {
		std::cerr << "Invalid argument passed to agent" << arg
			  << std::endl;
		exit(1);
	}
	auto ebpf_path = arg.substr(0, where);
	auto hooker_path = arg.substr(where + 1);
	setup_syscall_tracer(ebpf_path.c_str(), hooker_path.c_str());
